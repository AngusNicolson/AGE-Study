...
def find_k_nearest_patches_to_prototypes(dataloader,  # pytorch dataloader (must be unnormalized in [0,1])
    ...
    '''
    log(f"Finding {k} nearest patches for each prototype")
    log(f"{len(dataloader)} total batches")
    if (prototype_network is None) == (prototype_network_parallel is None):
        raise ValueError(f"Must specify only one of prototype_network ({prototype_network}) "
                         f"and prototype_network_parallel ({prototype_network_parallel}).")
    elif prototype_network is None:
        prototype_network_parallel.eval()
        model = prototype_network_parallel.module
    else:
        prototype_network.eval()
        model = prototype_network
        ...
        for img_idx, distance_map in enumerate(proto_dist_):
            for j in range(n_prototypes):
                ...
                if len(heaps[j]) < k:
                    ...
                    # followed by heappop
                    item = heapq.heappushpop(heaps[j], closest_patch)
                    del item
                del closest_patch
        del search_batch_input
        del search_batch
        del proto_dist_
