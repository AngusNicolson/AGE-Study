...
class ResNetFeatures(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False, layer_strides=None):
        ...
        self.block = block
        self.layers = layers
        self.layer1 = self._make_layer(block=block, planes=64, num_blocks=self.layers[0], stride=layer_strides[0])
        self.layer2 = self._make_layer(block=block, planes=128, num_blocks=self.layers[1], stride=layer_strides[1])
        self.layer3 = self._make_layer(block=block, planes=256, num_blocks=self.layers[2], stride=layer_strides[2])
        self.layer4 = self._make_layer(block=block, planes=512, num_blocks=self.layers[3], stride=layer_strides[3])
...
def resnet_features(name, pretrained=False, pretrained_path=None, **kwargs):
    model_block_types = {
        "resnet18": BasicBlock,
        "resnet34": BasicBlock,
        "resnet50": Bottleneck,
    ...
    if name not in model_blocks.keys():
        raise NotImplementedError(f"{name} not recognised for ResNet features")

    model = ResNetFeatures(model_block_types[name], model_blocks[name], **kwargs)
    if pretrained:
        if pretrained_path is None:
            my_dict = model_zoo.load_url(model_urls[name], model_dir=model_dir)
            my_dict.pop('fc.weight')
            my_dict.pop('fc.bias')
        else:
            my_dict = torch.load(pretrained_path)
            if "state_dict" in (my_dict.keys()):
                my_dict = my_dict["state_dict"]
            for k in list(my_dict.keys()):
                if "num_batches_tracked" in k:
                    my_dict[k] *= 0
            if "fc.1.weight" in my_dict.keys():
                my_dict.pop("fc.1.bias")
                my_dict.pop("fc.1.weight")
        model.load_state_dict(my_dict, strict=False)
    return model


def resnet18_features(**kwargs):
    return resnet_features("resnet18", **kwargs)


def resnet18_features_high_res1221(**kwargs):
    return resnet_features("resnet18", **kwargs)


def resnet18_features_high_res1211(**kwargs):
    return resnet_features("resnet18", **kwargs)


def resnet34_features(**kwargs):
    return resnet_features("resnet34", **kwargs)


def resnet50_features(**kwargs):
    return resnet_features("resnet50", **kwargs)


def resnet101_features(**kwargs):
    return resnet_features("resnet101", **kwargs)


def resnet152_features(**kwargs):
    return resnet_features("resnet152", **kwargs)


if __name__ == '__main__':

    r18_features = resnet18_features(pretrained=True, layer_strides=[1, 2, 2, 1])
    print(r18_features)

    x = torch.zeros((1, 3, 224, 224))
    output = r18_features(x)
    print(output.shape)
